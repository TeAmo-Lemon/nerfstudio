...
from data.datamanager import DataManager, DataManagerConfig
from scene.gaussian_model import SplatfactoModel, SplatfactoModelConfig
from utils import profiler


def module_wrapper(ddp_or_model: Union[DDP, SplatfactoModel]) -> SplatfactoModel:
    """
    If DDP, then return the .module. Otherwise, return the model.
    """
    if isinstance(ddp_or_model, DDP):
        return cast(SplatfactoModel, ddp_or_model.module)
    return ddp_or_model
...
class VanillaPipelineConfig(InstantiateConfig):
    ...
    model: SplatfactoModelConfig = field(default_factory=SplatfactoModelConfig)
...
class VanillaPipeline(nn.Module):
    ...
    def __init__(
        ...
        if world_size > 1:
            self._model = typing.cast(SplatfactoModel, DDP(self._model, device_ids=[local_rank], find_unused_parameters=True))
