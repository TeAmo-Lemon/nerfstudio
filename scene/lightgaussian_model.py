"""3D Gaussian Splatting + LightGaussian compression pipeline.
...
LightGaussian (Fan et al., 2023) compresses 3DGS via:
    1. Importance-based pruning — score each Gaussian by multi-view contribution,
       prune the lowest-scoring ones.
    2. SH distillation — teacher-student KD to reduce spherical harmonics degree.

Flow:
    init → setup gaussians → [training loop]:
        Stage 1 (standard): prepare_train_step → get_outputs → get_loss_dict →
            get_metrics_dict → finish_train_step (densify/prune + importance-prune)
        Stage 2 (distill, optional): same loop but with teacher rendering +
            distillation loss, reduced SH degree
"""
...
@dataclass
class LightGaussianModelConfig(InstantiateConfig):
    """Configuration for 3D Gaussian Splatting with LightGaussian compression."""

    _target: Type = field(default_factory=lambda: LightGaussianModel)
...
class LightGaussianModel(nn.Module):
    """3D Gaussian Splatting + LightGaussian compression — standalone implementation.

    Flow:
        init → setup gaussians → [training loop]:
            prepare_train_step →
            get_outputs:
                ① Standard RGB rasterization (gsplat)
                ② If in distill phase: teacher RGB rasterization (no grad)
                ③ Return {rgb, depth, accumulation, background, [teacher_rgb]}
            get_loss_dict:
                ① L1 + SSIM RGB loss
                ② Scale regularization
                ③ Distillation loss (during distill phase)
            finish_train_step:
                ① Standard densify/prune step
                ② Importance-based pruning at prune_iterations
                ③ Enter distillation mode at distill_start_iter
                ④ Logging
    """

    config: LightGaussianModelConfig
    ...
    def get_outputs(self, camera: Cameras) -> Dict[str, Union[torch.Tensor, List]]:
        """Render RGB + depth + optional teacher RGB from a camera viewpoint.

        During distillation phase, also renders the frozen teacher model
        ...
        # ⑮ teacher render (during distill phase)
    ...
    def _render_teacher(
        ...
        full_sh = self.config.sh_degree  # teacher uses full SH
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None) -> Dict[str, torch.Tensor]:
        """L1 + SSIM + scale regularization + optional distillation loss."""
    ...
    def _importance_prune(self, step: int) -> None:
        ...
        # Apply volume-aware adjustment if configured
        if self.config.prune_type == "v_importance":
            volume = torch.prod(torch.exp(self.scales), dim=1)
            # kth_percent_largest = 90th percentile volume
        ...
        mask[top_indices] = False  # mask=True means "prune this"
...
def _init_strategy(config: LightGaussianModelConfig, num_train_data: int):
