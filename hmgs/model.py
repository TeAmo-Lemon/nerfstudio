...
from hmgs.callbacks import TrainingCallback, TrainingCallbackAttributes, TrainingCallbackLocation
from hmgs.camera_optimizers import CameraOptimizer, CameraOptimizerConfig
from hmgs.cameras import CameraType, Cameras
from hmgs.colors import get_color
from hmgs.console import CONSOLE
from hmgs.math_utils import k_nearest_sklearn, random_quat_tensor
from hmgs.optimizers import Optimizers
from hmgs.scene_box import OrientedBox
from hmgs.spherical_harmonics import RGB2SH, SH2RGB, num_sh_bases

try:
    from hmgs.lib_bilagrid import BilateralGrid, color_correct, slice, total_variation_loss
except ImportError:
    BilateralGrid = None
    color_correct = None
    slice = None
...
class SplatfactoModel(Model):
    ...
    def populate_modules(self):
        ...
        if (
            ...
            shs = torch.zeros((self.seed_points[1].shape[0], dim_sh, 3), dtype=torch.float32, device=means.device)
        ...
        if self.config.use_bilateral_grid:
            if BilateralGrid is None:
                raise ImportError("Bilateral grid support requires tensorly. Install it or disable use_bilateral_grid.")
    ...
    def features_dino(self):
        if "features_dino" not in self.gauss_params:
            return torch.empty((0, self.config.dino_feature_dim), dtype=self.means.dtype, device=self.means.device)
    ...
    def _coerce_hmgs_camera(self, camera: Any) -> Cameras:
        if isinstance(camera, Cameras):
            return camera

        required_attrs = ("camera_to_worlds", "fx", "fy", "cx", "cy", "width", "height")
        if not all(hasattr(camera, attr) for attr in required_attrs):
            raise TypeError(f"Unsupported camera type for HMGS rendering: {type(camera)}")

        distortion_params = getattr(camera, "distortion_params", None)
        camera_type = getattr(camera, "camera_type", CameraType.PERSPECTIVE)
    ...
    def get_outputs(self, camera: Any) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        """
        camera = self._coerce_hmgs_camera(camera)
